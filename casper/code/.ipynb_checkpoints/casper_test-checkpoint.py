...
def rkf_validator(data, hyperparameters, n_splits, n_repeats, device='cpu', fast_mode=True):
    ...
    for i, (train_index, test_index) in enumerate(rkf.split(data)):
        ...
        # train the model
        train(casper_net, data_train, hyperparameters, task="regression", verbose=False)
...
def final_test(data, hyperparameters, n_splits, n_repeats, device='cpu'):
    """
    ...
    MSEs, L1Es, unique_labels, classwise_MSEs, classwise_L1Es = rkf_validator(data, hyperparameters, n_splits, n_repeats, device=device, fast_mode=False)
